...
[#postinstall2]
.Application Load Balancer SSL certificate setup
...
=== [Optional] Additionnal accounts creation

The 3decision stack will create a 3decision user named ``3decision`` (you can change this default name during the stack deployment). Additionnal accounts can be created by connecting to the admin UI: 

[start=1]
. Connect to the admin pannel

Connect to https://3decision-etl.<your domain>

[start=2]
. Navigate to users management

Navigate to Security/Users as shown in <<postinstall4>>.

Connect with the account ``3decision`` and the password you provided during the installation.

[#postinstall4]
.Users configuration panel
image::../docs/deployment_guide/images/users_config.png[Postinstall]

[start=3]
. Add users

Add users by specifing their user name and password.